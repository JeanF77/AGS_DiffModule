...
const PAGE_SIZE                               = 100; // ---- Nombre maximum d'artefacts à retourner par requete
...
/**
 * Generer code HTML du contenu d'une ligne de tableau
 * @param {Array} myTabVal - Tableau de valeurs a inserer dans une table HTML (facultatif, cree alors une ligne vide)
 * @returns {String} - Code HTML genere
 */

 function gui_genRow (myTabVal) {
   let myRow = "";
...
/**
 * Generer code HTML du contenu d'une ligne de tableau compact
 * @param {Object} myObject - Structure de valeurs a inserer dans une table HTML
 * @param {Boolean} myTypeChange - Indique si le rapport doit contenir la colonne "Old Artifact Type"
 * @param {Array} myAttrList - Liste d'attributs à afficher
 * @returns {String} - Code HTML genere
 */

function gui_genRow2 (myObject, myTypeChange, myAttrList) {
   let myRow = '<tr>';

   // ---- Contenu par défaut

   myRow += '<td>' + myObject.id + '</td><td>' + myObject.type + '</td><td>' + myObject.difflabel + '</td><td>' + myObject.diffcontent + '</td>';

   if (myTypeChange) { // ---- Le rapport doit contenir la colonne "Old Artifact Type"
      myRow += '<td>' + myObject.newtype + '</td>';
   }

   // ---- Ajouter les "Custom Attributes", s'il y en a

   for (let i=0; i < myAttrList.length; i++) {
      let myAttr = myObject.custattr.find(row => row.attrname === myAttrList[i]);

      if (myAttr === undefined) {
         myRow += '<td></td><td></td>';
      } else {
         myRow += '<td>' + myAttr.oldvalue + '</td><td>' + myAttr.newvalue + '</td>';
      }
   }
...
/**
 * Exporter le rapport de comparaison vers EXCEL
 * @param {Boolean} isCompact - export compacted report 
 * @returns 
 */
...
function export_report (isCompact) {
   ...
   myHtmlCode  = '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40">';
   myHtmlCode += '<head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]-->';
   myHtmlCode += '<meta charset="UTF-8" />';
   myHtmlCode += '<style>table, th, td {border: 1px solid black;}</style>';
   myHtmlCode += '</head><body>',
   myHtmlCode += '<p>Project Name = <b>' + g_Project.name + '</b></p>';
   myHtmlCode += '<p>New module = <b>' + g_ConfNew.name + '/' + g_ModuleNew.name + '</b></p>';
   myHtmlCode += '<p>Old module = <b>' + g_ConfOld.name + '/' + g_ModuleOld.name + '</b></p>';
   myHtmlCode += '<p>Date = <b>' + myDate.toString() + '</b></p>';
   myHtmlCode += '<p>Number of changes = <b>' + myFilteredTable.length + '</b></p>';
   ...
   myHtmlCode += '<table><thead><tr>';

   if (isCompact != undefined && isCompact === true) {
      console.log ("$$$ Compact Export Diff Report to EXCEL");

      // ---- Comptacter la table des différences (passer de la structure d'entrée à la structure de sortie)
      
      for (let i=0; i < myFilteredTable.length; i++) {
         let myCompactRow = myCompactTable.find(row => row.id === myFilteredTable[i][RPT_COL_ID]);

         if (myCompactRow === undefined) { // ---- Créer une nouvelle ligne si l'artefact n'a pas été retrouvé dans la table compacte
            let myRow = {
            ...
            // ---- Ajouter le label si pas encore existant dans la liste des différences

            if (myCompactRow.difflabel.indexOf(myFilteredTable[i][RPT_COL_DIFFLABEL]) === -1) {
               myCompactRow.difflabel += '<br>' + myFilteredTable[i][RPT_COL_DIFFLABEL];
            }
         }

         // ---- Mise en forme des données pour certain type de différence
         ...
         switch (myFilteredTable[i][RPT_COL_DIFFLABEL]) {
            ...
            case DIFF_LABEL[DIFF_ARTIFACTTYPECHANGED]:
               // ---- Détecter s'il y a des changemenst de type d'artefact
               myTypeChange = true;
            ...
            case DIFF_LABEL[DIFF_TAGCHANGED]:
               // ---- Détecter s'il y a des changemenst de "custom attributes" ou tags
               ...
               myCompactRow.custattr.push(myCustAttrDiff);

               myTableSpan = ' rowspan="2" colspan="1"';
               break;
      ...
      // ---- +----+------+------------+--------------+-----------+-----------+
      // ---- | ID | Type | Diff Label | New Type (*) | Custom Attribute  (*) |
      // ---- |    |      |            |              | Old Value / New Value |
      // ---- | 0  | 1    | 2          | 3            | 4         | 5         |
      // ---- +----+------+------------+--------------+-----------+-----------+
      // ---- (*) : not mandatory
      ...
      myHtmlCode += '<th'+ myTableSpan +'>ID</th>';
      myHtmlCode += '<th'+ myTableSpan +'>Type</th>';
      myHtmlCode += '<th'+ myTableSpan +'>Diff Label</th>';
      myHtmlCode += '<th'+ myTableSpan +'>Diff Content</th>';

      if (myTypeChange) { // ---- Si le rapport doit contenir des évènement de type "Changement de type d'artefact"
         myHtmlCode += '<th'+ myTableSpan +'>Old Artifact Type</th>';
      }

      if (myCustAttrChangeList.length > 0) { // ---- Si le rapport doit contenir la liste des attributs modifiés
         myCustAttrChangeList.sort();

         for (let i=0; i < myCustAttrChangeList.length; i++) {
            myHtmlCode += '<th colspan="2">' + myCustAttrChangeList[i] + '</th>';
         }

         myHtmlCode += '</tr><tr>';

         for (let i=0; i < myCustAttrChangeList.length; i++) {
            myHtmlCode += '<th>Old Value</th><th>New Value</th>';
         }
      }
      ...
      for (let i=0; i < myCompactTable.length; i++) {
         myHtmlCode += gui_genRow2 (myCompactTable[i], myTypeChange, myCustAttrChangeList);
      }

   } else {
      console.log ("$$$ Standard Export Diff Report to EXCEL");
      ...
      for (let i=0; i < myFilteredTable.length; i++) {
         myHtmlCode += gui_genRow (myFilteredTable[i]);
      }
   }

   myHtmlCode += '</tbody></table>';
   myHtmlCode += '</body></html>';

   // ---- Exporter la table temporaire
   // ----     download(data, strFileName, strMimeType)

   download (myHtmlCode, g_ModuleNew.name + '.html', 'text/html');
}
